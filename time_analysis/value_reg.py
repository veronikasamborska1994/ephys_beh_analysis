...
def run():  
     ...
     #perm_HP_all, perm_PFC_all = a_b_check(PFC, HP, n = 11, perm = 1000,task_check = 0, reward_times_to_choose = np.asarray([20,25,35,42]), c_1 = 1)
     ...
     #real_dif_PFC_0, real_dif_HP_0 = real_diff_a_b(HP, PFC, n = 11, task_check = 0, c_1 = 1, reward_times_to_choose= np.asarray([20,25,35,42]))
    ...
    # perms_b_all_0,perms_a_all_0 = plot(HP, PFC, c_1 = 1, n = 11,
   #                                  reward_times_to_choose = np.asarray([20,25,35,42]),task_check = 0)
...
#
        ...
        for i in range(perm):
            ...
            cpd_perm[i].append(re._CPD(X,y_perm).reshape(n_neurons, n_timepoints, n_predictors))
...
def run_perm_cpd(HP, PFC):
    cpd_HP,cpd_perm_HP =  perm_roll(HP, n = 11, perm = 1000)
    cpd_PFC,cpd_perm_PFC =  perm_roll(PFC, n = 11, perm = 1000)
