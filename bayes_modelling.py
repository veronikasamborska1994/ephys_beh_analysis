...
class model():
    ...
    def session_likelihood(self, session, params):
       ...
       # task_2 = np.where(task_non_forced == 2)[0]  
        ...
        # Variables.
        Posterior_correct_incorrect = np.zeros([n_trials, 2])  # Model free action values.
        #Posterior_correct_incorrect[0,:] =  0
        
        Prior_correct_incorrect = np.zeros([n_trials, 2])  # Model free action values.
        Prior_correct_incorrect[0,:] =  0.5
        ...
        for i, (o, c) in enumerate(zip(outcomes, switch_choice)): # loop over trials.
            if o == 1:
                # Decision is  correct based on the probability of getting a reward/
                ...
                Posterior_correct_incorrect[i, 0] = (1-reward)*Prior_correct_incorrect[i, 0]/(((1-reward)*Prior_correct_incorrect[i,0])+ (reward*Prior_correct_incorrect[i,1]))
                Posterior_correct_incorrect[i, 1] = 1-Posterior_correct_incorrect[i, 0]
        ...
        stay  =  switch
        trial_log_likelihood_switch = switch_choice*protected_log(choice_probs[:,1])
        trial_log_likelihood_stay = stay*protected_log(choice_probs[:,0])
...
session_fit_Q1 = fit_session(session = session, agent = model(), repeats = 5, brute_init = True, verbose = False)
