...
class Rsync_aligner():
    ...
    def __init__(self, pulse_times_A, pulse_times_B, chunk_size=5, plot=False):
        '''Find which pulses in sequences A and B correspond by aligning short chunks
        of sequence A with B by minimising the discrepancy (mean squared error) between
        ...
        if plot:
            ...
            plt.hist(timing_errors[~np.isnan(timing_errors)],np.arange(-10,10,0.1))
            plt.xlabel('Δt')
