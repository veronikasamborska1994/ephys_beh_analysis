...
def svd_trial(data,dm, n):
    ...
    # choice_id = 0
    #  and np.where(choices == choice_id)
...
def svd_diff_eig_same_data(data,dm, c, t, n, start, end):
    ...
    task_3_svd_1 = np.vstack((np.mean(svd_task_3_1[:,:,start:end],axis = 2),np.mean(svd_task_3_2[:,:,start:end],axis = 2)))
    task_3_svd_2 = np.vstack((np.mean(svd_task_3_3[:,:,start:end],axis = 2),np.mean(svd_task_3_4[:,:,start:end],axis = 2)))#   
...
#                             np.transpose(svd_task_3_4,[0,2,1]).reshape(svd_task_3_4.shape[0]*svd_task_3_4.shape[2],svd_task_3_4.shape[1])))
    ...
    #Task 1 and 2
    s_task_within = np.linalg.multi_dot([u_t1_1.T, task_1_svd_2.T, vh_t1_1.T])
    s_withing_1_2 = s_task_within.diagonal()     
    sum_within_1_2 = np.cumsum(abs(s_withing_1_2))/task_1_svd_2.T.shape[0]
    
    s_task_between = np.linalg.multi_dot([u_t2_1.T, task_1_svd_2.T, vh_t2_1.T])
    s_between_1_2 = s_task_between.diagonal()     
    sum_c_between_1_2 = np.cumsum(abs(s_between_1_2))/task_1_svd_2.T.shape[0]
   
    #Task 2 and 3

    s_task_between_2_3 = np.linalg.multi_dot([u_t3_1.T, task_2_svd_2.T, vh_t3_1.T])
    s_2_3 = s_task_between_2_3.diagonal()     
    sum_between_2_3 = np.cumsum(abs(s_2_3))/task_2_svd_2.T.shape[0]

    s_task_within_2_3 = np.linalg.multi_dot([u_t2_1.T, task_2_svd_2.T, vh_t2_1.T])
    s_within_2_3 = s_task_within_2_3.diagonal()     
    sum_within_2_3 = np.cumsum(abs(s_within_2_3))/task_2_svd_2.T.shape[0]

    within = np.mean([sum_within_1_2, sum_within_2_3], 0)
    between = np.mean([sum_c_between_1_2, sum_between_2_3], 0)
...
def svd(data,dm, c, t, n, start, end):
    ...
    task_m = np.vstack((task_1_svd_1,task_1_svd_2,task_2_svd_1,task_2_svd_2,task_3_svd_1,task_3_svd_2))
    
    corr_m = np.corrcoef(task_m)
    plt.figure()
    plt.imshow(corr_m)
...
def plotting():
    ...
    svd(Data_PFC, DM_PFC, 'red', 'PFC reward',  10 , 42,52)
    svd(Data_PFC, DM_PFC, 'pink', 'PFC choice', 10, 30,40)
    svd(Data_PFC, DM_PFC, 'darkred', 'PFC initiation', 10, 20,30)

    svd(Data_HP, DM_HP, 'blue', 'HP reward', 10, 42,52)
    svd(Data_HP, DM_HP, 'lightblue', 'HP choice', 10, 30,40)
    svd(Data_HP, DM_HP, 'darkblue', 'HP initiation', 10, 20,30)
    

    svd_diff_eig_same_data(Data_PFC, DM_PFC, 'red', 'PFC reward',  10 , 42,52)
    svd_diff_eig_same_data(Data_PFC, DM_PFC, 'pink', 'PFC choice', 10, 30,40)
    svd_diff_eig_same_data(Data_PFC, DM_PFC, 'darkred', 'PFC initiation', 10, 20,30)

    svd_diff_eig_same_data(Data_HP, DM_HP, 'blue', 'HP reward', 10, 42,52)
    svd_diff_eig_same_data(Data_HP, DM_HP, 'lightblue', 'HP choice', 10, 30,40)
    svd_diff_eig_same_data(Data_HP, DM_HP, 'darkblue', 'HP initiation', 10, 20,30)
